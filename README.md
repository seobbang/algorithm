...
| 241121 | 백준 - 평범한 배낭(12865\*) #DP                                                          |
| 241122 | 백준 - 동전(9084) #DP                                                          |
| 241124 | 백준 - 가장 긴 감소하는 수열(11722\*) #DP                                                          |
| 250226 | 백준 - 좌표 압축(18870\*) #좌표압축                                                      |
| 250227 | 백준 - 스택 수열(1874) #스택                                                      |
| 250301 | 백준 - 구간 합 구하기(11660\*) 5 #DP,누적합                                                  |
| 250302 | 백준 - 바이러스(2606), 숨바꼭질3(13549\*) #BFS                                             |
| 250305 | 백준 - 안전영역(2468), BOJ거리(12026) #BFS                                             |
| 250306 | 백준 - 전쟁(1303), 음식물 피하기(1743), 숨바꼭질2(12851)  #BFS                                             |
| 250307-250308 | 백준 - 숨바꼭질4(13913), 이모티콘(14226), 퇴사2(15486), 달리기(16930), A->B(16953), 아기 상어2(17086)  #BFS, DP                                             |
| 250310 | 백준 - 1, 2, 3 더하기 4(15989)  #DP                                             |
| 250408 | 백준 - 에디터(1406)  #Stack                                             |
...
[시간 복잡도](https://www.bigocheatsheet.com/) : O(NlogN)
best: O(N) (거의 정렬되어 있는 경우)

Chrome: 병합정렬(Merge sort)과 삽입정렬(Insert sort)의 하이브리드 버전인 타임정렬(Time sort)을 내부 알고리즘으로 사용
...
- i 번째 물건 무게가 j 보다 작거나 같으면)<br/>
  Max(<br/>
  `i번째거 안들어가는 경우` `1~i-1`번째 물건으로 구한 최대값, <br/>
  `i번째거 들어가는 경우` `1~i-1`번째 물건을 사용해 **i번째 무게를 뺀** 만큼 만들어진 최대값 + i번째 물건 가치값<br/>)
...
**누적합**
- index 0 ~ N 까지의 누적합을 저장해두고, 구간 i ~ j를 구할 때 sum[j] - sum[i] 으로 구하는 방법.
- 2차원 배열일때는 ?
  1. 누적합 구하기
    ```js
...
- BFS: 너비 우선 탐색
  ...
  - 인접 리스트
    ...
    ```
  -> `if(!visited[nextV])` 가중치가 있는 경우에는 아예 방문 X가 아니라 값 비교후 갱신으로 가야함.
...
#### 🔵 좌표 압축
> 여러 좌표들의 범위가 클 때 작은 범위의 인덱스로 변환하는 기법

- `2 4 -10 4 -9` value값을 정렬 후,<br/>
`(-10, 0) (-9, 1) (2, 2), (4, 3)`와 같이 (문제 조건에 따라 중복 제거) `(value, index)` 값을 만들어주고 (`Map` 유용) <br/>
원래의 숫자 순서대로 살피며 index 값만을 내뱉으면 `2, 3, 0, 3, 1` 이렇게 index 값만 남게됨!
